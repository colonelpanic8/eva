...
/**
 * Opt-in live test of the whole voice action path: synthetic speech replaces the
 * microphone, the real controller advertises the real catalog, the model delegates a
 * tool call, and the real Android backend hands off to the Clock app.
 */
@RunWith(AndroidJUnit4::class)
class NativeVoiceActionLiveTest {
    @Test
    fun spokenTimerRequestExecutesThroughTheControllerAndSurvivesTheHandoff() {
            ...
            runBlocking(Dispatchers.Main.immediate) {
                ...
                val controller =
                    ProviderSessionController(
                        ...
                        voiceProviderFactory = { brokerLink, audio ->
                            BrokerConversationProvider(
                                BrokerEndpoint.parse(brokerLink),
                                offerSdp = audio.createOffer(),
                                onAnswer = audio::acceptAnswer,
                            )
                        },
                    )
                ...
                try {
                    ...
                    instrumentation.sendStatus(
                        ...
                        Bundle().apply {
                            putString(
                                "evaVoiceActionEvidence",
                                "PASS spoken request, delegated tool call, timer handoff, spoken confirmation",
                            )
