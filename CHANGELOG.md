...
- Hanging up no longer kills the app. A turn that finished within milliseconds of the call ending stopped the background work service before Android had delivered its first `onStartCommand`, so `startForeground` never ran and the system killed the process with `ForegroundServiceDidNotStartInTimeException`. A stop that arrives that early is now held and applied by the service once it is actually foreground. The voice session service shared the same shape and got the same guard.
