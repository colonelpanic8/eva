...
- Other apps: recent messaging notifications and their explicit text-reply
  actions. No target-app changes, plugins, Shizuku, or app-specific package
  allowlist are required.

Ivan reported a verified real SMS send on 2026-09-14. That verifies direct
sending; it is distinct from the Messages JSON plugin's previously verified
...
The shared interface is deliberately independent of plugin files. Future
